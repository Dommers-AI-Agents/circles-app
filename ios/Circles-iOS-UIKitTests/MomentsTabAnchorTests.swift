...
struct MomentsTabAnchorTests {
    ...
    @Test func slotFillsWhatIsLeftUnderTheBarAndTheSectionPinsToTheTop() {
        let layout = MomentsTabAnchor.layout(visibleHeight: 750, sectionTop: 380, chromeHeight: 90, topInset: 100)
    ...
    @Test func aDailyCardAboveTheSlotCountsAsChrome() {
        let without = MomentsTabAnchor.layout(visibleHeight: 750, sectionTop: 380, chromeHeight: 90, topInset: 100)
        let with = MomentsTabAnchor.layout(visibleHeight: 750, sectionTop: 380, chromeHeight: 90 + 120, topInset: 100)
    ...
    @Test func theReelIsNeverSquashedBelowTheMinimum() {
        let layout = MomentsTabAnchor.layout(visibleHeight: 300, sectionTop: 0, chromeHeight: 90, topInset: 0)
        #expect(layout.slotHeight == 320)
    }

    @Test func fractionalHeightsRoundDownSoPagesNeverOvershoot() {
        let layout = MomentsTabAnchor.layout(visibleHeight: 750.6, sectionTop: 0, chromeHeight: 90.3, topInset: 0)
        #expect(layout.slotHeight == 660)
    }
}
