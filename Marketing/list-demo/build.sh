#!/bin/bash
# build.sh — assemble the Google-Maps-list demo (copied from share-demo/build.sh;
# no action-row rings here). Captions/cards are HTML→Chrome→PNG overlays,
# narration beats placed at the driver's marks without overlapping, dead
# space auto-cut, brand cards (frame 0 is the poster — no fade-in) wrapped
# around the walkthrough.
...
# A back-tap that was swallowed and retried: the retry is the real navigation,
# so don't protect (keep) the dead window around the first attempt.
if "back-to-profile-retry" in marks:
    marks.pop("back-to-profile", None)
